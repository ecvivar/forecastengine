...
import { api, CalibrationReport, CalibrationBin } from "@/lib/api";
...
export default function MonitoringPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/v1/audit/log").then((r) => r.json()).catch(() => [] as AuditEntry[]),
      fetch("/api/v1/monitoring/drift").then((r) => r.json()).catch(() => null),
          ...
          {auditLog.length > 0 ? (
            <div className="space-y-2">
              {auditLog.slice(0, 20).map((entry) => (
                <div key={entry.id} className="flex items-start gap-3 py-2 border-b border-[hsl(var(--border))] last:border-0 text-sm">
                  <Clock className="w-3 h-3 mt-0.5 text-[hsl(var(--muted))]" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <Badge variant="default" className="text-2xs">{entry.action}</Badge>
                      <span className="text-xs text-[hsl(var(--muted))]">{formatDateTime(entry.timestamp)}</span>
                    </div>
                    <div className="text-xs text-[hsl(var(--foreground))] mt-0.5 truncate">{entry.details}</div>
                  </div>
            ...
            <div className="text-sm text-[hsl(var(--muted))] italic py-4 text-center">
              No audit entries available. Ensure the backend audit endpoint is accessible.
            </div>
          )}
        </CardContent>
      </Card>
